...
// --- Color Definitions (Modern Purple Tone) ---
const CATEGORY_STYLES = {
    // Label backgrounds & Text colors
...
const ProfitManager = () => {
    ...
    const openModal = (date) => {
        ...
        // Default Markdown header if note is empty
        if (!data.note) {
                ...
                <div className="flex flex-col gap-0.5">
                    ...
                    {hasEvent ? (
                        <div className="group relative flex justify-between items-center px-1.5 py-1 rounded text-[10px] font-bold mb-1 bg-white border-l-4 border-[#7C4DFF] shadow-sm text-[#7C4DFF]">
                         ...
                         <div className="h-[22px] mb-1"></div> /* Spacer to keep alignment */
                    )}

                    {/* Layers 2-5: Financials */}
                    {data ? (
                        <>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-fade-in-up">
                <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handleExportMD} className="text-xs flex items-center gap-1 text-slate-600 hover:text-[#7C4DFF] border border-slate-300 rounded px-3 py-1.5 bg-white hover:bg-slate-50 transition-colors shadow-sm font-medium">
                        ...
                        return (
                            <React.Fragment key={wIdx}>
                                <div className="cal-label-col border-r border-b border-slate-100">
                                    <div className="h-[26px] mb-1"></div> {/* Spacer for Event layer */}
                        ...
                        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                            ...
                            <div className="flex-1 flex flex-col h-full bg-white">
                                <div className="flex border-b border-slate-200 shrink-0">
                                    <button onClick={() => setModalTab('edit')} className={`px-6 py-3 text-sm font-bold ${modalTab === 'edit' ? 'text-[#7C4DFF] border-b-2 border-[#7C4DFF]' : 'text-slate-500 hover:bg-slate-50'}`}>編集 (Markdown)</button>
                                    <button onClick={() => setModalTab('preview')} className={`px-6 py-3 text-sm font-bold ${modalTab === 'preview' ? 'text-[#7C4DFF] border-b-2 border-[#7C4DFF]' : 'text-slate-500 hover:bg-slate-50'}`}>プレビュー</button>
                                </div>
                                <div className="flex-1 overflow-hidden relative">
                                    {modalTab === 'edit' ? (
                                        <textarea className="w-full h-full p-4 resize-none outline-none font-mono text-sm leading-relaxed" value={editData.note} onChange={(e) => setEditData({...editData, note: e.target.value})} />
                                    ) : (
