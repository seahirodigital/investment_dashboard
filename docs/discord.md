...
| 日本株セクター資金流入通知 | `C:\Users\mahha\OneDrive\開発\investment_dashboard\.github\workflows\sector_category_discord.yml` | 平日12:35 JSTの前場終了後と15:40 JSTの大引け後。各時刻から10分間隔で4回監視 | あり。`mode=jp` | 各日・各時間帯で最初の成功時だけ送信。失敗時は次の監視回が再試行 | 当日の日付、セクター資金流入割合分析、上位5件、下位5件、ハッシュタグ、セクター分析ページURL、MooView資金フロー | 1. 日本株セクター全ランキング<br>2. 日本株セクター上位7件チャート＋ランキング<br>3. 日本株セクター下位7件チャート＋ランキング<br>4. JPセクター/TPX<br>5. 半導体関連/TPX | 成功マーカーArtifactで重複送信を防止 |
...
| 日本株セクター通知の監視 | 12:35/12:45/12:55/13:05 JSTと15:40/15:50/16:00/16:10 JSTに起動し、未配信の場合だけ処理する |
