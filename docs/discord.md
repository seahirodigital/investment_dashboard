...
| 米国株セクター資金流入通知 | `C:\Users\mahha\OneDrive\開発\investment_dashboard\.github\workflows\sector_category_discord.yml` | 06:25 JST（UTC月〜金の21:25なのでJST火〜土相当） / `25 21 * * 1-5` UTC | あり。`mode=us` | 常に送信 | 当日の日付、SPY相対の米国株各セクター上位7件・下位7件、ハッシュタグ、US個別分析ページURL | 1. 米国株セクター上位7件チャート＋ランキング<br>2. 米国株セクター下位7件チャート＋ランキング | 成功。直近手動実行: 2026-06-07 JST / `https://github.com/seahirodigital/investment_dashboard/actions/runs/27086990720` |
| 日本株セクター資金流入通知 | Oracle実行本体: `C:\Users\mahha\OneDrive\開発\investment_dashboard\scripts\market\jp_sector_discord_oracle.py` / Actions: `C:\Users\mahha\OneDrive\開発\investment_dashboard\.github\workflows\sector_category_discord.yml` | Oracleは平日11:35 JSTに`--slot midday`、平日15:35 JSTに`--slot close`。Actionsの日本株定期実行は一時停止中 | Actionsは`mode=jp`で手動実行可能。Oracle手動実行あり | Oracle側は各日・各時間帯で最初の成功時だけ送信。ローカルJSONマーカーで重複防止 | 当日の日付、セクター資金流入割合分析、上位5件、下位5件、ハッシュタグ、セクター分析ページURL、MooView資金フロー | 1. 日本株セクター全ランキング<br>2. 日本株セクター上位7件チャート＋ランキング<br>3. 日本株セクター下位7件チャート＋ランキング<br>4. JPセクター/TPX<br>5. 半導体関連/TPX | Actionsの11:35〜12:05 JSTおよび15:45〜16:15 JSTの定期起動だけを一時停止。Oracle版は継続 |
...
| 日本株セクター通知の監視 | Actionsの11:35/11:45/11:55/12:05 JSTと15:45/15:55/16:05/16:15 JSTの定期起動は一時停止中。`mode=jp`の手動実行は維持 |
