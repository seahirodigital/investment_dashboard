...
| 日本株セクター資金流入通知 | Oracle実行本体: `C:\Users\mahha\OneDrive\開発\investment_dashboard\scripts\market\jp_sector_discord_oracle.py` | Oracleは平日11:35 JSTに`--slot midday`、平日15:35 JSTに`--slot close`。既存Actionsは安定確認用に残す | Oracle手動実行あり | Oracle側は各日・各時間帯で最初の成功時だけ送信。ローカルJSONマーカーで重複防止 | 当日の日付、セクター資金流入割合分析、上位5件、下位5件、ハッシュタグ、セクター分析ページURL、MooView資金フロー | 1. 日本株セクター全ランキング<br>2. 日本株セクター上位7件チャート＋ランキング<br>3. 日本株セクター下位7件チャート＋ランキング<br>4. JPセクター/TPX<br>5. 半導体関連/TPX | 安定確認中はActionsとOracleの重複通知を許容 |
