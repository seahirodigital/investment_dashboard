...
NIKKEI225_HEATMAP_PATTERN = re.compile(r"(日経225|日本225)\s*ヒートマップ")
...
def _capture_nikkei225_heatmap_image(page, output_path: Path) -> Path:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    heatmap_table = (
        page.locator("table.tbl")
        .filter(has_text=NIKKEI225_HEATMAP_PATTERN)
        .filter(has_text="業種別指数")
        .first
    )
...
def capture_nikkei225_chart_assets(output_dir: Path) -> dict[str, Any]:
    ...
    for attempt in range(1, 4):
        captured_heatmap: Path | None = None
        captured_contribution: Path | None = None
        try:
                ...
                try:
                    ...
                    page.wait_for_function(
                        """() => Array.from(document.querySelectorAll('table.tbl')).some((table) => {
                          ...
                          const hasHeatmapTitle = text.includes('日経225 ヒートマップ')
                            || text.includes('日本225 ヒートマップ');
                          return hasHeatmapTitle && text.includes('業種別指数');
                        })""",
                        timeout=45000,
                    )
                    try:
                        captured_heatmap = _capture_nikkei225_heatmap_image(page, heatmap_image)
                    except Exception as exc:
                        last_error = exc
                        print(
                            "[警告] 日経225ヒートマップ画像の取得に失敗しました "
                            f"({attempt}/3): {exc}"
                        )
                    ...
                    try:
                        ...
                        captured_contribution = _capture_table_without_caption(
                            page,
                            contribution_image,
                            table_id="nkrnk",
                            padding=0,
            ...
            last_error = exc
            print(
                "[警告] 日経225ヒートマップページの取得に失敗しました "
                f"({attempt}/3): {exc}"
            )

        images = [path for path in [captured_heatmap, captured_contribution] if path]
        if captured_heatmap:
            return {
                "images": images,
                "heatmap_image": captured_heatmap,
                "contribution_image": captured_contribution,
            }
        if attempt < 3:
            print("   [情報] 日経225ヒートマップ取得を5秒後に再試行します。")
            time.sleep(5)

    print(
        "[警告] 日経225ヒートマップを取得できないため、"
        "本文の目次前ヒートマップを省略し、サムネイルはランキング画像へフォールバックします: "
        f"{last_error}"
    )
    return {
        "images": [],
...
def _apply_affiliate_links(
    ...
    affiliate_manager = _load_module(
        "investment_dashboard_affiliate_runtime",
...
def publish_note_blog(
    ...
    heatmap_thumbnail_source = _first_existing_image_path([nikkei225_assets.get("heatmap_image")])
    if heatmap_thumbnail_source:
        thumbnail_source = heatmap_thumbnail_source
        print(f"   [情報] noteサムネイルに日経225ヒートマップを使用します: {thumbnail_source}")
    else:
        ...
        if thumbnail_source:
            print(
                "   [警告] 日経225ヒートマップが取得できないため、"
                f"ランキング系画像をnoteサムネイルに使用します: {thumbnail_source}"
            )
