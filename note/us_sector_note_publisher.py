...
INTRO_TEXT = (
    "米国株のランキング（セクター）を元に、今日上昇・下落の可能性がある日本株銘柄分析の参考情報を提供します。"
    "米国で上昇・下落セクターと、日本での各セクターの上下動が相関する研究を踏まえています。"
    "日本取引時間内の、日本株ETFでおすすめ上位銘柄や、2026年の今後の見通し、"
    "今後上がる・伸びる銘柄投資分析のために、米国時間推移とチャートを用い、分析をまとめます。\n\n"
    "Trading Viewの無料版ではSP500の割返した複数銘柄を１つのチャートに表示できないため、"
    "多忙な方はフォローいただき、毎日以下のSP500で割返したランキングを見るだけでも、"
    "マクロ環境への意識が違って来ると思います。"
)
DISCLOSURE_TEXT = (
    "Amazonのアソシエイトとして本アカウントは適格販売により収入を得ています。"
    "文章にはAIの整形・編集が含まれ、解釈は自己責任でお願いします。"
)
SECTOR_RANKING_EXPLANATION_TEXT = (
    "以下は、米国市場での各セクターETFをSPY（SP500）で割返すことで、"
    "マクロや市場情報を排除した際に、各セクターへの資金流入を分析しています。"
    "米国市場での資金流入・流出と日本市場での相関があることを踏まえ、"
    "本日の東証の取引時間での各セクターへの資金の流れの参考になります。"
)
FOLLOW_TEXT = (
...
def _post_to_note(
    ...
    try:
        ...
        publish_button_patch = _load_module(
            "investment_dashboard_us_sector_publish_button_patch",
