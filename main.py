...
def main_jpx():
    ...
    try:
        ...
        print(f"\nJPXエラーが発生しました: {e}")
        # JPX処理のみ失敗しても、後続の主体別スクレイピングは実行したい場合は
        # ここで例外を再送出せず、エラーログを出して終了する
        traceback.print_exc()
        # Github Actionsを失敗ステータスにしたい場合は再送出すべきだが、
        # 今回は「主体別」の実行も重要なので、ここでは握りつぶして続行させる運用とする
        pass
    finally:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # モジュールをリロードして最新の状態にする（念のため）
        import importlib
        importlib.reload(shutai_scraper)
        
        shutai_scraper.scrape_shutai_data()
        
    except ImportError as e:
        print(f"!!! Failed to import shutai_scraper: {e}")
        print("Check if 'shutai_scraper.py' exists in the root directory.")
    except Exception as e:
