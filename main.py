...
def extract_date_from_filename(url):
    ...
    重要: ファイル名の日付は**その週の金曜日（週末）**を示している
    例: stock_val_1_231208.pdf → 2023年12月8日（金）→ その週の月曜日は12月4日
    
    2022年のデータは除外する
    """
    try:
        # ファイル名から日付部分を抽出 (例: 231208)
        ...
        if match:
            ...
            print(f"  ファイル日付: {file_date.strftime('%Y-%m-%d')} ({weekday_names[weekday]}) → 週開始日(月): {monday.strftime('%Y-%m-%d')}")
...
def process_historical_data():
    ...
    if DEBUG_MODE:
        print("\n" + "="*60)
        print("⚠️  DEBUG MODE ACTIVE - 2023年の最初の5件のみ取得")
        print("="*60 + "\n")
    else:
        print("\n=== 過去データの取得を開始 ===")
    ...
    if DEBUG_MODE:
        # デバッグモード: 2023年の最初の5件のみ取得
        urls_2023 = get_all_pdf_urls_by_year(2023)
        # 古い順にソート（最初の5件 = 2023年初期のデータ）
        all_urls = sorted(urls_2023)[:DEBUG_LIMIT]
        print(f"デバッグモード: 2023年の最初の {len(all_urls)} 件のPDFを処理します")
    else:
        # 本番モード: 全年度のデータを取得
        all_urls = []
